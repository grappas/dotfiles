...
>/tmp/eww/cpu_usage_per_core_bar_collection
cat /tmp/eww/mpstat.out.per_core_raw | while read A B; do
    if ( ($A <= $B)); then
        MAX_USAGE=$B
    else
        MAX_USAGE=$A
    ...
    CORE_FREQUENCY=$(cat /tmp/eww/zenmonitor.out | grep "Core $CORE_COUNTER Frequency" | sed 's/^.*Frequency\t//g' | sed 's/\.//g')
    ...
    echo "$LINE_TO_PRINT" >>/tmp/eww/cpu_usage_per_core_bar_collection
    CORE_COUNTER=$(($CORE_COUNTER + 1))
done

cat /tmp/eww/cpu_usage_per_core_bar_collection
