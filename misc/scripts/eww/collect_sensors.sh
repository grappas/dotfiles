...
cat /tmp/eww/mpstat.out | grep all | while read all usr nice sys iowait irq soft steal guest gnice idle; do
    echo $((100 - $(echo $idle | sed 's/\..*$//g'))) >/tmp/eww/cpu_usage.out
done

>/tmp/eww/mpstat.out.per_core.temp

cat /tmp/eww/mpstat.out | grep -v all | grep -v CPU | while read CPU CORE SOCK NODE usr nice sys iowait irq soft steal guest gnice idle; do
    if ((($(echo $idle | sed 's/\.//g') % 100) >= 50)); then
...
seq 0 $(($(grep ^cpu\\scores /proc/cpuinfo | uniq | awk '{print $4}') - 1)) | while read each; do
...
BLACHA=0
for each in $(cat /tmp/eww/zenmonitor.out | grep -i frequency | grep -v -i effective | sed 's/Core\ [0-9]\ Frequency\t//g' | sed 's/\.//g' | sed 's/...$//g'); do
    if [ $BLACHA -le $each ]; then
        BLACHA=$each
    fi
done
